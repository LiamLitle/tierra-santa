  window.addEventListener("load", () => {
    ...
    /* ─────────────────────────────────────────
       ...
       (the Gallery page's .gallery-grid tiles keep a
       tighter stagger between photos — lots of them in
       view at once, so a wide cascade would feel slow —
       but every fade now takes the same 1s to complete)
    ───────────────────────────────────────── */
    gsap.utils.toArray('.place-img, .experience-image, .guide-image').forEach((el, i) => {
      const inGalleryGrid = el.closest('.gallery-grid');
      gsap.fromTo(el,
        { opacity: 0, scale: 0.96, filter: 'blur(22px)' },
        { opacity: 1, scale: 1,    filter: 'blur(0px)',
          duration: 1,
          ease: 'power2.out',
          delay: inGalleryGrid ? i * 0.02 : i * 0.07,
...
(function () {
  function init() {
    ...
    // Generic: any .place-img gallery on any page (was scoped to #place/#sacred-alliance
    // only, but The Sanctuary subpage reuses the same gallery markup for its own rooms).
    document.querySelectorAll('.place-img img').forEach(img => {
